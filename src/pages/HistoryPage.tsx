      ...
      series: [
        {
          ...
          markLine: {
            ...
            label: { color: '#f59e0b', fontSize: 11 },
          },
        },
      ],
    };
    ...
    <div className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800">面试历史记录</h1>
        </div>
        <Link
          to="/"
      ...
      {records.length >= 2 && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 mb-6">
          <h2 className="text-base font-semibold text-gray-700 mb-1">📈 成长曲线</h2>
          <p className="text-xs text-gray-400 mb-4">每次面试的AI评分趋势</p>
      ...
      {records.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-500 mb-2">暂无面试记录</h3>
          <p className="text-sm text-gray-400 mb-6">完成一次模拟面试后，记录将显示在这里</p>
          <Link
            to="/"
            className="inline-flex px-5 py-2.5 bg-indigo-500 hover:bg-indigo-600 text-white font-medium rounded-xl transition-colors"
          >
            去面试
          </Link>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  ...
                  <th className="text-left py-3 px-4 text-xs font-medium text-gray-500">场景</th>
                  <th className="text-left py-3 px-4 text-xs font-medium text-gray-500 hidden sm:table-cell">详情</th>
                  <th className="text-center py-3 px-4 text-xs font-medium text-gray-500">评分</th>
                  <th className="text-center py-3 px-4 text-xs font-medium text-gray-500">时长</th>
                  ...
                  <tr key={rec.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-center">
                      <span className={`inline-block font-bold text-sm ${
                        rec.aiScore >= 80 ? 'text-green-600' : rec.aiScore >= 60 ? 'text-amber-600' : 'text-red-500'
                      }`}>
                        {rec.aiScore}
                      </span>
                    </td>
                        ...
                        {confirmDelete === rec.id ? (
                          <span className="flex items-center gap-1">
                            <button
                              ...
                              className="px-2 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50 rounded-lg"
                            >
                              确认
                            </button>
                            <button
