    ...
    <div className="flex-1">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-14 sm:pb-20">
        <div className="text-center mb-10">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#DADBD6]">选择模拟场景</h2>
        ...
        <div className="text-center mt-8">
          <Link to="/history" className="inline-flex items-center gap-2 text-sm text-[#6B6B68] hover:text-[#5CA98A] transition-colors group">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            查看面试历史记录
          </Link>
        </div>
      </section>
      <section className="border-t border-[#3A3A38]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
          <div className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#DADBD6]">为什么选择面面俱到？</h2>
            <p className="text-[#6B6B68] mt-2 text-sm">三大核心能力，让你的面试准备更高效</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {[{icon:'🎙️',title:'语音实时交互',desc:'AI面试官语音播报问题，你用语音回答——右侧面板同步显示实时字幕。'},{icon:'🎯',title:'智能追问引擎',desc:'AI根据你的行业、岗位和每一条回答，实时分析并生成针对性追问。'},{icon:'📊',title:'六维数据复盘',desc:'六维雷达图+面部表情分析+逐题回答分析，精准定位提升方向。'}].map((item, i) => (
              <div key={i} className="text-center p-6 rounded-2xl bg-[#2A2A28] border border-[#3A3A38]">
                <div className="text-3xl mb-3">{item.icon}</div>
                <h3 className="font-semibold text-[#DADBD6] mb-2">{item.title}</h3>
