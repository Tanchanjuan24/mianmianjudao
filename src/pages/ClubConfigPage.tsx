...
export default function ClubConfigPage() {
  ...
  return (
    <div className="flex-1 max-w-2xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
      {/* 顶部 */}
      <div className="mb-8">
        <Link to="/" className="inline-flex items-center gap-1 text-sm text-[#8A9B8F] hover:text-[#FBC64D] transition-colors mb-4">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-[#FBC64D] to-[#FBC64D] rounded-xl flex items-center justify-center text-white">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
            </svg>
          </div>
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-[#1A2B20]">社团竞选配置</h1>
            <p className="text-sm text-[#8A9B8F]">选择竞选目标，开启模拟答辩</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#CDD2CC] shadow-sm p-5 sm:p-7 space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-[#1A2B20] mb-2">
            组织类型 <span className="text-[#C96B5E]">*</span>
          </label>
            ...
            {ORG_TYPES.map((org) => (
              <button
                ...
                className={`py-2.5 px-3 rounded-lg text-sm font-medium border transition-all ${
                  orgType === org
                    ? 'border-[#FBC64D] bg-[#FCF3DC] text-[#FBC64D] shadow-sm'
          ...
          {errors.orgType && <p className="text-[#C96B5E] text-xs mt-1.5">{errors.orgType}</p>}
        </div>

        {/* 竞选职位 */}
        <div>
          <label className="block text-sm font-medium text-[#1A2B20] mb-2">
            竞选职位 <span className="text-[#C96B5E]">*</span>
          </label>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        ...
        <div>
          <label className="block text-sm font-medium text-[#1A2B20] mb-2">
            个人陈述 <span className="text-[#8A9B8F] font-normal">（可选）</span>
          </label>
          <textarea
            ...
            className="w-full border border-[#CDD2CC] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#FBC64D] focus:ring-1 focus:ring-[#FBC64D] resize-none transition-colors"
          />
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-[#1A2B20] mb-2">
            其他说明 <span className="text-[#8A9B8F] font-normal">（可选）</span>
          </label>
          <textarea
            value={notes}
        ...
        <button
          onClick={handleStart}
          className="w-full py-3 bg-gradient-to-r from-[#FBC64D] to-[#FBC64D] hover:from-[#FBC64D] hover:to-[#FBC64D] text-white font-semibold rounded-xl text-base transition-all shadow-md hover:shadow-lg active:scale-[0.98]"
        >
